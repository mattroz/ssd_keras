...
class DataGenerator(keras.utils.Sequence):
    ...
    idxly provides three methods to parse annotation data: A general-purpose CSV parser,
    ...
    def parse_csv(self,
       ...
       self.returns:
        ...
        idx_file = data[0][0] # The idx image for which we're collecting the ground truth boxes
        idx_image_id = data[0][0].split('.')[0] # The image ID will be the portion of the image name before the first dot.
        idx_labels = [] # The list where we collect all ground truth boxes for a given image
        add_to_dataset = False
        for i, box in enumerate(data):

            if box[0] == idx_file: # If this box (i.e. this line of the CSV file) belongs to the idx image file
                idx_labels.append(box[1:])
                if i == len(data)-1: # If this is the last line of the CSV file
                    if random_sample: # In case we're not using the full dataset, but a random sample of it.
                        ...
                        if p >= (1-random_sample):
                            self.labels.append(np.stack(idx_labels, axis=0))
                            self.filenames.append(os.path.join(self.images_dir, idx_file))
                            self.image_ids.append(idx_image_id)
                    else:
                        self.labels.append(np.stack(idx_labels, axis=0))
                        self.filenames.append(os.path.join(self.images_dir, idx_file))
                        self.image_ids.append(idx_image_id)
                ...
                if random_sample: # In case we're not using the full dataset, but a random sample of it.
                    ...
                    self.filenames.append(os.path.join(self.images_dir, idx_file))
                    self.image_ids.append(idx_image_id)
                idx_labels = [] # Reset the labels list because this is a new file.
                idx_file = box[0]
                idx_image_id = box[0].split('.')[0]
                idx_labels.append(box[1:])
                if i == len(data)-1: # If this is the last line of the CSV file
                    if random_sample: # In case we're not using the full dataset, but a random sample of it.
                        p = np.random.uniform(0,1)
    ...
    def parse_xml(self,
        ...
        the code, but in its idx form it expects the data format and XML tags of the Pascal VOC datasets.
    ...
    def parse_json(self,
        ...
        the code, but in its idx form it expects the JSON format of the MS COCO datasets.
    ...
    def create_hdf5_dataset(self,
                            ...
                            verbose=True):
        '''
        Converts the idxly loaded dataset into a HDF5 file. This HDF5 file contains all
    ...
    def __getitem__(self, idx):
        ...
        Arguments:
             self.batch_size (int, optional): The size of the batches to be generated.
            ...
            transformations (list, optional): A list of transformations that will be applied to the images and labels
                ...
                and optionally labels (also as a Numpy array) andself.returns an image and optionally labels in the same
                format.
           self.label_encoder (callable, optional): Only relevant if labels are given. A callable that takes as input the
                labels of a batch (as a list of Numpy arrays) andself.returns some structure that represents those labels.
           ...
           self.returns (set, optional): A set of strings that determines what outputs the generator yields. The generator's output
        ...
        if isinstance(self.label_encoder, (tuple)):
            self.label_encoder = self.label_encoder[0]

        if isinstance(self.returns, (tuple)):
            self.returns = self.returns[0]

        if isinstance(self.degenerate_box_handling, (tuple)):
            self.degenerate_box_handling = self.degenerate_box_handling[0]

        if isinstance(self.keep_images_without_gt, (tuple)):
            self.keep_images_without_gt = self.keep_images_without_gt[0]

        if isinstance(self.shuffle, (tuple)):
            self.shuffle = self.shuffle[0]
        ...
        #############################################################################################
        # Warn if any of the setself.returns aren't possible.
        #############################################################################################
        ...
        if self.labels is None:
            if any([ret in self.returns for ret in ['original_labels', 'processed_labels', 'encoded_labels', 'matched_anchors', 'evaluation-neutral']]):
                warnings.warn("Since no labels were given, none of 'original_labels', 'processed_labels', 'evaluation-neutral', 'encoded_labels', and 'matched_anchors' " +
                              "are possibleself.returns, but you set `returns = {}`. The impossibleself.returns will be `None`.".format(self.returns))
        elif self.label_encoder is None:
            if any([ret in self.returns for ret in ['encoded_labels', 'matched_anchors']]):
                warnings.warn("Since no label encoder was given, 'encoded_labels' and 'matched_anchors' aren't possibleself.returns, " +
                              "but you set `returns = {}`. The impossibleself.returns will be `None`.".format(self.returns))
        ...
        if not (self.labels is None):
            if isinstance(self.transformations, (tuple)):
                self.transformations = self.transformations[0]
        ...
        #    files from disk.
        batch_indices = self.dataset_indices[idx:idx+ self.batch_size]
        if not (self.images is None):
            for i in batch_indices:
                batch_X.append(self.images[i])
            if not (self.filenames is None):
                batch_filenames = self.filenames[idx:idx+ self.batch_size]
            else:
        ...
        if not (self.labels is None):
            batch_y = deepcopy(self.labels[idx:idx+ self.batch_size])
        else:
            batch_y = None

        if not (self.eval_neutral is None):
            batch_eval_neutral = self.eval_neutral[idx:idx+ self.batch_size]
        else:
            batch_eval_neutral = None

        # Get the image IDs for this batch (if there are any).
        if not (self.image_ids is None):
            batch_image_ids = self.image_ids[idx:idx+ self.batch_size]
        else:
        ...
        if not (self.label_encoder is None or self.labels is None):
    ...
    def save_dataset(self,
        ...
        '''
        Writes the idx `filenames`, `labels`, and `image_ids` lists to the specified files.
